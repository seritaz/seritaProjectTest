...
JNIEXPORT jint JNICALL Java_jni_Jni_patch(JNIEnv *env, jstring oldPath, jstring newPath,
                                          jstring patchPath) {
    const char *pOldPath = env->GetStringUTFChars(oldPath, JNI_FALSE);
    const char *pNewPath = env->GetStringUTFChars(newPath, JNI_FALSE);
    const char *pPatchPath = env->GetStringUTFChars(patchPath, JNI_FALSE);
    ...
    env->ReleaseStringUTFChars(oldPath, pOldPath);
    env->ReleaseStringUTFChars(newPath, pNewPath);
    env->ReleaseStringUTFChars(patchPath, pPatchPath);
    return ret;
}
